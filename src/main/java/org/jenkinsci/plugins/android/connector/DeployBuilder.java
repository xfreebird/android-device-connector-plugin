...
public class DeployBuilder extends Builder {
    ...
    @Inject
    private transient AndroidDeviceList devices;
    ...
    public boolean perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener) throws InterruptedException, IOException {
        ...
        if (files.length == 0) {
            listener.getLogger().println("No Android apps found to deploy!");
            return false;
        }
        ...
        for (FilePath bundle : files) {
            ...
            if (idx < 0) {
                listener.getLogger().printf("Ignoring '%s'; expected .apk\n", name);
                continue;
            }

            listener.getLogger().printf("Deploying Android app: %s\n", name);
    ...
    public static class DescriptorImpl extends BuildStepDescriptor<Builder> {
        ...
        public String getDisplayName() {
            return "Deploy Android App to device";
        }
